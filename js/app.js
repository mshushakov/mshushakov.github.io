...
_restdb2.default.get('examples').then(data => {
	...
	_component2.default.render(new Cards({ words, examples }), document.querySelector('.viewport'));
});
...
class Card extends _component2.default {
	...
	save(e) {
		...
		_restdb2.default.post('examples', { example: value, word: this.data.word }).then(data => {
			...
			examples.push({ example: value, word: this.data.word });
			elem.value = '';
			this.bindings.setData({
				state: '-saved',
				examples
	...
	render() {
		return `<div class="card">
                <div class="_content">
                    ...
                    <div class="_headline">${this.data.word}</div>
                    <div class="_subheading"></div>
                    <div class="_description">
                    	<ol data-bind="each:examples"></ol>
                    </div>
...
class Cards extends _component2.default {
  ...
  this.update();*/
		...
		this.data.words.map(word => {
			_component2.default.render(new Card({ word, examples: this.data.examples.filter(item => item.word === word) }), this.cards);
		});
...
//Component.render(new Cards({ words: ['勉強する'], examples:[{ example: '勉強してください', word: '勉強する' }] }), document.querySelector('.viewport'));
...
class Binder {
    ...
    add(element, ...components) {
        components.forEach(component => {
            ...
            el.textContent = component.example;
